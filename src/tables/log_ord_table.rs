...
///
/// 默认的等待写入日志文件的事务最大数量限制
///
const DEFAULT_WAIT_WRITE_LOG_TRANSACTION_MAX_LIMIT: usize = 512;

///
/// 默认的等待写入日志文件的事务超时时长，单位ms
///
const DEFAULT_WIAT_WRITE_LOG_TRANSACTION_TIMEOUT: usize = 60 * 1000;
    ...
    fn commit(&self, confirm: <Self as Transaction2Pc>::CommitConfirm)
        ...
        async move {
            ...
            if tr.is_require_persistence() {
                ...
                self.0.table.0.rt.spawn(self.0.table.0.rt.alloc(), async move {
