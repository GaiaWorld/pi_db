...
use crate::{Binary, KVAction, KVActionLog, KVDBCommitConfirm, KVTableTrError, TableTrQos, TransactionDebugEvent, transaction_debug_logger,
            ...
            tables::KVTable};
use crate::tables::log_ord_table::{LogOrderedTable, LogOrdTabTr};
...
> KVTable for BtreeOrderedTable<C, Log> {
    ...
    fn collect(&self) -> BoxFuture<Result<(), Self::Error>> {
        ...
        async move {
            ...
            table.unlock_write(write_lock_uid); //持久化提交成功，则释放写锁
...
> BtreeOrderedTable<C, Log> {
    ...
    /// 获取当前B树表的唯一写锁，成功返回写锁的唯一ID，失败返回空
    pub(crate) fn lock_write(&self) -> Option<u64> {
        if self.0.write_lock.load(Ordering::Acquire) == UNLOCKED_WRITE_FLAG {
            ...
            if let Ok(UNLOCKED_WRITE_FLAG) = self.0.write_lock.compare_exchange(UNLOCKED_WRITE_FLAG, write_lock_uid,
                                                                                Ordering::AcqRel,
                                                                                Ordering::Acquire) {
                //写锁成功
                Some(write_lock_uid)
    ...
    // 安全的解除表的写锁，成功返回真
    pub(crate) fn unlock_write(&self, write_lock_uid: u64) -> bool {
        ...
        async move {
            if tr.is_writable() {
                ...
                if let Some(inner_transaction) = &*tr.0.inner_transaction.lock() {
                    //有内部事务
                    if inner_transaction.is_write_conflict() {
                        //同时只允许有一个可写事务
                        return Err(<Self as Transaction2Pc>::PrepareError::new_transaction_error(ErrorLevel::Normal,
                                                                                                 format!("Prepare b-tree ordered table conflicted, table: {:?}, source: {:?}, transaction_uid: {:?}, prepare_uid: {:?}, reason: write conflict",
                                                                                                         self.0.table.name().as_str(),
                                                                                                         self.0.source,
                                                                                                         self.get_transaction_uid(),
                                                                                                         self.get_prepare_uid())));
        ...
        async move {
            if let Some(inner_transaction) = &mut *tr.0.inner_transaction.lock() {
                match inner_transaction.delete(&key) {
                    ...
                    Ok(last_value) => {
                        let _ = tr.0.actions.lock().insert(key.clone(), KVActionLog::Write(None)); //记录对指定关键字的最新插入或更新操作
                        Ok(last_value)
                    },
                }
            } else {
...
> Drop for InnerBtreeOrdTabTr<C, Log> {
    fn drop(&mut self) {
        if let Some(write_lock_uid) = self.write_lock.take() {
            //当前是写事务，则解锁
            let _ = self.table.unlock_write(write_lock_uid);
        }
        ...
        loop {
            if let Some(lock) = table.lock_write() {
                ...
                match table.0.inner.read().begin_write() {
                    Err(e) => {
                        //写入redb失败，则稍候重试
                        table.unlock_write(lock); //立即释放当前B树表的写锁
                        table
                            .0
                            .collecting
